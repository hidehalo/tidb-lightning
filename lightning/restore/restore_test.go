...
var _ = Suite(&restoreControllerSuite{})

type restoreControllerSuite struct {
	rc *RestoreController
}

func (s *restoreControllerSuite) TestRestoreSchema(c *C) {
	// ctx := context.Background()
	// s.mockDB.
	...
	// err := InitSchema(ctx, 1, s.rc.tiGlue.GetParser(), s.tiGlue.GetSQLExecutor(), "db", map[string]string{
